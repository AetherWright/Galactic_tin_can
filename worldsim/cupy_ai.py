"""CuPy-accelerated AI controllers for WorldSim.
...
def _ensure_cupy():
    ...
    if getattr(xp, '__name__', '') != 'cupy':
        raise RuntimeError('CuPy backend is required for worldsim.cupy_ai')
    return xp
...
class SimplePerceptron:
    ...
    (GeLU).  This keeps the module numerically stable on both NumPy and CuPy
    backends and continues to expose the simple Hebbian update rule used by the
...
class NelderMeadPolicy:
    ...
    def choose_action(self, state: Sequence[float]) -> int:
        ...
        best_idx = 0
        best_val = values[0]
        for idx in range(1, valid_actions):
...
class WarAI(NelderMeadPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        epsilon: float = 0.1,
        gamma: float = 0.95,
        hidden_layers: Sequence[int] = (20, 16, 12, 9, 6, 4),
        memory_size: int = 32,
    ) -> None:
...
class DomesticPolicyAI(NelderMeadPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        epsilon: float = 0.1,
        gamma: float = 0.95,
        hidden_layers: Sequence[int] = (12, 9, 7, 5, 4),
    ) -> None:
        super().__init__(
            n_inputs,
            actions,
...
class ProjectAI(NelderMeadPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        epsilon: float = 0.1,
        gamma: float = 0.95,
        hidden_layers: Sequence[int] = (14, 11, 9, 7, 5, 4),
    ) -> None:
        super().__init__(
            n_inputs,
            actions,
...
class DiplomacyAI(NelderMeadPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        epsilon: float = 0.1,
        gamma: float = 0.9,
        hidden_layers: Sequence[int] = (10, 8, 6, 4, 3),
    ) -> None:
        super().__init__(
            n_inputs,
            actions,
