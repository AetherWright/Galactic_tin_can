"""Military AI: doctrine-driven FSM for fleets and a post-civilian DoctrineAI.

Architecture overview
---------------------

DoctrineAI
    Runs at the **end** of each nation's ``process_turn``, *after* the
    civilian and diplomacy AIs have already executed.  It therefore sees the
    civilian AI's latest action choice (``nation.last_civilian_action``) and
    the current alliance / war state shaped by diplomacy.  Its output is a
    :class:`DoctrineSignal` stored on ``nation.doctrine_signal``.

FleetFSM
    Each fleet holds a lightweight neural controller
    (:class:`LayeredNetworkFleetController`, or :class:`TorchFleetController`
    when PyTorch is available).  Every fifth ``fsm_transition`` is called:
...
Division doctrine integration
    ``issue_orders`` in :mod:`worldsim.models.war` reads
    ``nation.doctrine_signal`` and applies doctrine-level overrides *on top of*
    the WarAI's tactical recommendation (e.g. ``TOTAL_WAR`` forces all
    divisions to attack regardless of the WarAI output).

Modifying the system
--------------------
* **New doctrine**  — add a value to :class:`DoctrineSignal`, extend
  :data:`DOCTRINE_FSM_BIAS` and :data:`_DIVISION_DOCTRINE_OVERRIDE`, then
  update :data:`DOCTRINE_LIST`.
...
from ..ai import LayeredNetwork, NelderMeadPolicy
...
# Ordered lists — append-only so existing saved models stay valid.
...
def make_fleet_controller() -> LayeredNetworkFleetController:
    ...
    Prefers :class:`~worldsim.torch_ai.TorchFleetController` when PyTorch is
    importable; otherwise returns :class:`LayeredNetworkFleetController`.
    """
    try:
        from ..torch_ai import TorchFleetController
...
def fsm_execute_state(
    ...
    from .fleet import order_fleet_move
...
def issue_doctrine(
    ...
    *,
    ...
    ``issue_orders`` call in :mod:`worldsim.models.war` at the start of each
